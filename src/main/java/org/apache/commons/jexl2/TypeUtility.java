...
public class TypeUtility {
    ...
    public static HashSet set(Object[] args) {
        ...
        if ( anon != null) {
            for (Object obj : list) {
                anon.jexlContext.set("$_", obj);
                Object v = anon.block.jjtAccept(anon.pv,null);
                set.add(v);
            }
        }else{
            for (Object obj : list) {
    ...
    private static Object sqlmath(Object[] argv) {
        ...
        if ( list.size() > 0  ){
            int index = 0 ;
            math[0] = Double.valueOf( list.get( index ).toString() );
            math[1] = math[0];
            math[2] = math[0];
            for ( index = 1; index < list.size() ; index++ ){
                Double value = Double.valueOf( list.get( index ).toString() );
                if ( value < math[0]) {
                    math[0] = value ;  // MIN
                }
                if ( value > math[1]) {
